...
def read_PSB_csv(date_start, date_end, col_idx1, col_idx2):
    file_ = r'./SLC PSB/PSB_Meters4.csv' #adding file name

    #Reading data from csv and converting to a matrix format
    ...
    #Choosing data according to the dates
    start_idx, end_idx = PSB_select_date(datetime_mat, date_start, date_end)
    datetime_select = datetime_mat[start_idx:end_idx+1]
    delta, datetime_new = PSB_time_array(datetime_select) #making sure that the timesteps are 'spaced'
...
def read_muliple_csv(col_idx1, col_idx2):
    folder_path = r'/Home/Documents/SLC PSB data/SLC PSB'
...
def read_weather_files():
    folder_path = r'./WBB Weather Data/WBB'
    allfiles = glob.glob(folder_path + "/*.csv")
    #final_data = numpy.zeros((1, col_idx2-col_idx1))
...
def arrange_array(X, timestep_array, timestep_total):
    ...
    for i in range(0, len(timestep_array)):
        X_new[int(timestep_array[i]), :] = X[i, :]
...
def read_multiple_dataport(folder_path, num):
    ...
    for file_ in sorted(allfiles):
        ...
        if count==0:
            ...
            final_data = final_data + data_arr

        count = count + 1

        H_min, H_max = get_normalize_params(final_data)

    return final_data, H_max
