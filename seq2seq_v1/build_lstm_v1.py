...
def lstm_model_110(lstm_hidden, in_dim, tsteps):
    ...
    #lstm_hidden -> hyper-paremeteters.
    #in_dim = |X|
    #tsteps -> 24 hours, I'm dealing with sequences of 24 hours
    ...
    batch_size = 1
    #Here I will build the graphical model for deep RNN
    #inputs = [None, tsteps, in_dim]
    inputs = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))  #Layer 1
    encoder = LSTM(lstm_h1, activation='hard_sigmoid', W_regularizer=l2(0.0), return_sequences=False, stateful=True)(inputs) #Layer 2

    #Adding a dense layer
    #encoder = Dense(lstm_h1, activation='relu')(encoder)
    encoding_repeat = RepeatVector(tsteps)(encoder) #[None, in_dim] -> [None, tsteps, in_dim] #Layer 3
    decoder = LSTM(lstm_h2, return_sequences=True, activation='hard_sigmoid', W_regularizer=l2(0.01), stateful=True)(encoding_repeat) #Layer 3
    #decoder = ActivityRegularization(l2=0.01)(decoder)
    decoder = merge([decoder, inputs], mode='concat', concat_axis=2) #This is what I want to input


    #Creatinbg graph for Multi-layer perceptron
    input_MLP = Input(shape=(tsteps, lstm_h2+in_dim), batch_shape=(1, tsteps, lstm_h2+in_dim)) #Layer 4
    x = TimeDistributed(Dense(lstm_h3,  activation=activ_l3))(input_MLP) #Layer 4
    predictions_MLP = TimeDistributed(Dense(1, activation=activ_l4))(x)  #Layer 5, 6
    model_MLP = Model(input=input_MLP, output=predictions_MLP)
    ...
    out_3D = Reshape((tsteps, ))(predictions) #might have to reshape
    model = Model(inputs, out_3D)
    model.compile(loss='mean_squared_error', optimizer='adam') #Using adam, variant of SGD
