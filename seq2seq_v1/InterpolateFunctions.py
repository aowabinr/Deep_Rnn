...
def interpolate_main(X, Y, start_day, end_day, cons_points):
    ...
    #cons_points is the number of points I will accept linear interpolation
    ...
    #I created a fictitious missing value block here to test how the code is doing. you dont need that
    ...
    #small_list: list of indices of array used for linear interpolation
    #large_list: list of indices of array used for neural network (i.e. large chunk)

    return small_list, large_list


def interpolate_main_v2(Y, cons_points):
...
def optimize_LSTM(X, Y):
    ...
    def objective(params):
        ...
        #optimize_model = build_lstm_v1.lstm_model_109(params, X.shape[2], 24)
        for ep in range(10):
...
def LSTM_main(X, Y, X_e, best, intv):
    ...
    #lstm_model = build_lstm_v1.lstm_model_109(lstm_hidden, X.shape[2], tsteps)
...
def interpolate_energydata(train_data, H_t, start_day, end_day, cons_points):
    H1 = H_t.copy()
    small_list, large_list = interpolate_main(train_data, H_t, start_day, end_day, cons_points)
    H_t = interp_linear(H_t, small_list) #H_t is beign changed as numpy arrays are mutable
    H1 = interp_linear(H1, small_list)

    #PlotFunctions.Plot_interp_params()
    H_t = H_t[:, None] #changing numpy array shape to fit the function
    Y_t, Y_NN = interp_LSTM(train_data, H_t, large_list)
